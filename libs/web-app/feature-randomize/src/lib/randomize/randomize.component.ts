...
export class RandomizeComponent implements OnInit {
  ...
  generateHenchmen(starterDeck: IHenchmen[] = []): void {
    if (starterDeck.length === 0) {
      ...
      if (this.scheme.requiredCards !== undefined) {
        if (this.scheme.requiredCards !== undefined) {
          if (
            this.scheme.requiredCards.inVillainDeck.cardType ===
            CardType.HENCHMEN
          )
            starterDeck.push(
              this.scheme.requiredCards.inVillainDeck as IHenchmen
            );
        }
      }
  ...
  generateScheme(): void {
    ...
    this.bystanders = this.scheme.rules.villainDeck.numBystanders[
      this.numPlayers
    ];
  }
