...
function testGameSet(gameSet: IGameSet) {
  const setup: GameSetup = new GameSetup(gameSet, 2);
  const game:IGameSetup = setup.generateGame();
  describe(`Test ${gameSet.name} setup`, () => {
    it(`should only contain ${gameSet.name} cards`, () => {
      ...
      expect(game.heroDeck.every((item) => item.gameSet === gameSet)).toBeTruthy();
...
describe('Class creation', () => {
  it('should create with all Gamesets', () => {
    expect(new GameSetup(GameSets.ALL, 2)).toBeTruthy();
  });

  it('should fail with no Gameset', () => {
    expect(() => {
      const gameSetup = new GameSetup(undefined, 2);
    }).toThrow(Error);
  });

  it('should fail with an empty Gameset', () => {
    expect(() => {
      const gameSetup = new GameSetup([], 2);
    }).toThrow(Error);
  });
});

describe('Game creation', () => {
  const setup: GameSetup = new GameSetup(GameSets.ALL, 2);
  const game: IGameSetup = setup.generateGame();
  ...
  it('should have some heroes', () =>
    expect(game.heroDeck.length).toBeGreaterThan(0));
