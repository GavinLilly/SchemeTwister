...
export class Scheme implements IPlayableObject {
  ...
  private static _buildCards<T extends CardGroup>(
  ...
  private static _buildFromSeeds<T extends CardGroup>(
  ...
  private static _buildFromRandomCards<T extends CardGroup>(
    numRequiredCards: number,
    store: Readonly<CardStore<T>>
  ): ReadonlyArray<T> {
    ...
    return store.pickRandom({ count: numRequiredCards });
  }
