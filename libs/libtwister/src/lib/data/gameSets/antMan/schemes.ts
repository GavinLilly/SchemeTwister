...
import { CHIVALROUS_DUEL, EMPOWERED, SIZE_CHANGING } from './keywords';
import { META } from './meta';

class AntManScheme extends AbstractScheme {
  public readonly gameSetId = META.id;
}

export const AGE_OF_ULTRON = new AntManScheme(
  {
  ...
  11
).overrideEachRule((rule, numPlayer) => {
  if (numPlayer === 4) {
...
export const PULL_EARTH_INTO_MEDIEVAL_TIMES = new AntManScheme(
  {
...
export const TRANSFORM_COMMUTERS_INTO_GIANT_ANTS = new AntManScheme(
  {
...
export const TRAP_HEROES_IN_THE_MULTIVERSE = new AntManScheme(
  {
