...
class McuGardiansOfTheGalaxyScheme extends AbstractScheme {
  public readonly gameSetId = META.id;
}

export const INESCAPABLE_KYLN_SPACE_PRISON = new McuGardiansOfTheGalaxyScheme(
  {
...
export const PROVOKE_THE_SOVEREIGN_WAR_FLEET = new McuGardiansOfTheGalaxyScheme(
  {
...
class StarLordsAwesomeMixTapeScheme extends McuGardiansOfTheGalaxyScheme {
  public async getSetup(
    ...
    const guardians = heroStore.availableCards.filter(
      (hero) => hero.team === GUARDIANS_OF_THE_GALAXY
    );
    const guardianHero = randomize(guardians, 1)[0];
    const guardianHeroPicked = heroStore.getOne(guardianHero.id);
...
export const STAR_LORDS_AWESOME_MIX_TAPE = new StarLordsAwesomeMixTapeScheme(
  {
...
export const UNLEASH_THE_ABILISK_SPACE_MONSTER =
  new McuGardiansOfTheGalaxyScheme(
    {
