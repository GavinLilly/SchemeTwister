...
class IntoTheCosmosScheme extends AbstractScheme {
  public readonly gameSetId = META.id;
}

export const THE_CONTEST_OF_CHAMPIONS = new IntoTheCosmosScheme(
  {
...
class TurnTheSoulOfAdamWarlockScheme extends IntoTheCosmosScheme {
  public async getSetup(
    ...
    const adam = (await import('./heroes')).ADAM_WARLOCK;

    const adamPicked = heroStore.getOne(adam.id);
...
export const TURN_THE_SOUL_OF_ADAM_WARLOCK = new TurnTheSoulOfAdamWarlockScheme(
  {
...
class DestroyTheNovaCorpsScheme extends IntoTheCosmosScheme {
  public async getSetup(
    numPlayers: NumPlayers,
    selectedMastermind: AbstractMastermind,
    heroStore: MultiCardStore<IHero>,
    ...
    const novas: IHero[] = heroStore.availableCards.filter((hero) =>
      hero.name.toLowerCase().includes('nova')
    );

    if (novas.length > 0) {
      const chosenIdx = Math.floor(Math.random() * novas.length);
      const chosenNova = heroStore.getOne(novas.splice(chosenIdx, 1)[0].id);

      novas.forEach((nova) => heroStore.removeCard(nova.id));
      ...
      throw new Error('No Nova card is available to be selected');
    }
  }
}

export const DESTROY_THE_NOVA_CORPS = new DestroyTheNovaCorpsScheme(
  {
...
export const ANNIHILATION_CONQUEST = new IntoTheCosmosScheme(
  {
